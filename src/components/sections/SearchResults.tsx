...
function SearchResultsContent() {
  ...
  useEffect(() => {
    async function fetchPosts() {
      ...
      setLoading(true);
      try {
        const res = await fetch(`/api/posts?search=${encodeURIComponent(query)}&page=${page}&limit=${ITEMS_PER_PAGE}`);
    ...
    return (
      <div className="text-center py-32 bg-white rounded-[3rem] border-4 border-dashed border-gray-100">
        ...
        <p className="text-gray-400 font-bold text-lg tracking-wide">
          {query ? `هیچ بابەتێک بۆ گەڕانی "${query}" نەدۆزرایەوە` : 'تکایە وشەیەک بنووسە بۆ گەڕان'}
        </p>
    ...
    <div className="space-y-12">
      <div className="bg-[#563a4a] text-white p-6 rounded-2xl flex items-center justify-between shadow-xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-white/60 text-xs font-bold uppercase tracking-wider mb-1">ئەنجامەکانی گەڕان بۆ</p>
            <h4 className="text-xl font-black">{query}</h4>
          </div>
        </div>
        <div className="text-sm font-bold bg-white/10 px-4 py-2 rounded-lg">
          {totalCount} بابەت دۆزرایەوە
        </div>
      </div>
